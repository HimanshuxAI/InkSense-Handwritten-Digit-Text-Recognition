...
class DataLoaderIAM:
    ...
    def __init__(self, data_dir, batch_size, data_split=0.95, fast=False):
        """Loader for dataset."""
        self.fast = fast
        if fast and lmdb:
        ...
        f = open(str(data_dir) + '/gt/words.txt')
        chars = set()
        ...
        for line in f:
            ...
            file_name = str(data_dir) + '/img/' + file_name_subdir1 + '/' + file_name_subdir2 + '/' + file_base_name
    ...
    def _get_img(self, i):
        if self.fast and lmdb:
            with self.env.begin() as txn:
                basename = self.samples[i].file_path.split('/')[-1]
